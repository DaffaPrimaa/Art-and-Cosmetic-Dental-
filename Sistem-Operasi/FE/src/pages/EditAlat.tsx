...
export interface Alat {
  ...
  harga: number;
  keterangan: string;
  tanggal: string | null; // <-- 1. DITAMBAHKAN (string | null)
}
...
export default function EditAlat() {
  ...
  const [formData, setFormData] = useState<Alat>({
    ...
    harga: 0,
    keterangan: "",
    tanggal: null, // <-- 2. DITAMBAHKAN (default null)
  });
  ...
  useEffect(() => {
    const fetchAlat = async () => {
      try {
        ...
        // 3. PENTING: Format tanggal dari backend (YYYY-MM-DDTHH:MM:SS)
        // atau (YYYY-MM-DD) agar bisa dibaca input type="date"
        if (data.tanggal) {
  ...
  const handleChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "jumlah" || name === "harga"
          ? Number(value)
          : // 'tanggal' akan masuk ke 'value' biasa, yang sudah benar
            value,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 4. PENTING: Siapkan data untuk dikirim
    // Jika tanggal dikosongkan (value=""), kirim 'null' ke backend
    const dataToSend = {
      ...formData,
      tanggal: formData.tanggal === "" ? null : formData.tanggal,
    };
    ...
    try {
      const res = await fetch(`http://localhost:8000/alat/${id}`, {
        ...
        body: JSON.stringify(dataToSend), // <-- Kirim data yang sudah disiapkan
      });
  ...
  return (
    <div className="bg-white p-6 rounded shadow-md mb-6 mt-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* <-- 5. BLOK TANGGAL DITARUH DI PALING ATAS --> */}
        <div>
          <label className="block font-medium">Tanggal</label>
          <input
            name="tanggal"
            type="date"
            // PENTING: "|| ''" untuk mencegah error 'controlled vs uncontrolled'
            // jika formData.tanggal kebetulan 'null'
